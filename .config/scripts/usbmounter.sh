...
[ $rofi = true ] && menu='wofi --dmenu --prompt' || menu='dmenu -p' # define menu system

if [ $mount = true ]; then
  mountable=$(lsblk -lp | grep "part $" | awk '{print $1 " ("$4")"}') # get all not mounted partitions with their sizes
  chosen=$(echo "$mountable" | $menu "Mount partition " | cut -d " " -f 1) # let user choose one partition
  ...
  if [ $traditional = true ]; then # if traditional is set to true disk will be mounted by command mount so program will ask user where to mount a disk
    mounting_point=$(echo "" | $menu "Write folder where disk will be mounted ") # ask user for mounting_point
    ...
    if [ ! -d "$mounting_point" ]; then # if directory doesn't exist ask user for permission to create it
      answer=$(printf "y\nn" | $menu "Folder doesn't exist, do you want to create it?")

      if [ "$answer" = "y" ]; then 
        mkdir "$mounting_point" # create a mounting_point
      else 
  ...
  chosen=$(echo "$unmountable" | $menu "Unmount partition" | cut -d " " -f 1) # ask user for partition to unmount
