...
generate_menu() {
  echo -en "img:$GENERATE_DB_ICON\x00info:!Update Database\x1fGENERATE_DB\n"
  ...
  while IFS=',' read -r hash name path; do
    ...
    [[ -f "$thumb" ]] && echo -en "img:$thumb\x00info:$name\x1f$path\n"
  done < "$DB_FILE"
}
...
main() {
  ...
  case "$selected" in
    *"generate_db_thumbnail.png")
      echo "Updating database..."
      generate_db
      main
      return
      ;;
    #*"generate_db_thumbnail.png") echo "Updating database..."; generate_db;;
    *"shuffle_thumbnail.png")
      if [[ -s "$DB_FILE" ]]; then
      ...
      ;;
    *)
      base=$(basename "$selected" .png)
      hash=$(echo "$base" | awk -F'__' '{print $NF}')
      original_path=$(awk -F',' -v h="\"$hash\"" '$1 == h {gsub(/"/, "", $3); print $3}' "$DB_FILE")
      [[ -f "$original_path" ]] && set_wallpaper "$original_path" || notify-send "Wallpaper Error" "Original file not found."
      ;;
  esac
}
